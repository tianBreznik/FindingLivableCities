...
class VAE(nn.Module):
    def __init__(self, img_size, latent_spec, temperature=.67, use_cuda=False):
        ...
        self.hidden_dim = 289  # Hidden dimension of linear layer
        self.reshape = (289, 17, 17)  # Shape required to start transpose convs
        ...
        # Intial layer
        encoder_layers = [
            nn.Conv2d(self.img_size[0], 143, (4, 4), stride=2, padding=0),
            nn.ReLU(),
            nn.Conv2d(143, 143, (4, 4), stride=2, padding=1),
            nn.ReLU()
        ]

        encoder_layers += [
            nn.Conv2d(143, 289, (4, 4), stride=2, padding=1),
            nn.ReLU(),
            nn.Conv2d(289, 289, (4, 4), stride=2, padding=1),
            nn.ReLU()
        ]
        ...
        self.features_to_hidden = nn.Sequential(
            nn.Linear(289 * 17 * 17, self.hidden_dim),
            nn.ReLU()
        )
        ...
        self.latent_to_features = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(self.hidden_dim, 289 * 17 * 17),
            nn.ReLU()
        )
        ...
        decoder_layers += [
            nn.ConvTranspose2d(289, 289, (5, 5), stride=2, padding=0),#
            nn.ReLU(),#
            nn.ConvTranspose2d(289, 143, (3, 3), stride=2, padding=1),
            nn.ReLU(),
            nn.ConvTranspose2d(143, 143, (3, 3), stride=2, padding=1),
            nn.ReLU(),
            nn.ConvTranspose2d(143, self.img_size[0], (3, 3), stride=2, padding=1),
            nn.Sigmoid()
        ]
    ...
    def decode(self, latent_sample):
        ...
        #print("decode feets")
        #print(features.shape)
        #images_of_feet = self.features_to_img(features.view(-1, *self.reshape))
        #print("feet size")
        #print(features.view(-1, *self.reshape).shape)
        #print("and pixels")
        #print(self.img_size[0])
        #print(images_of_feet.shape)
